...
\svnidlong
{$HeadURL: svn+ssh://ruckdeschel@svn.r-forge.r-project.org/svnroot/distr/pkg/distrDoc/vignettes/distr.Rnw $}
{$LastChangedDate: 2016-04-23 18:26:35 +0200 (Sa, 23 Apr 2016) $}
{$LastChangedRevision: 1103 $}
{$LastChangedBy: ruckdeschel $}
%\svnid{$Id: distr.Rnw 1103 2016-04-23 16:26:35Z ruckdeschel $}
...
\code{p.r} and \code{q.l} for functions \code{p} and \code{q}, respectively, 
alluding to the right (resp. left) continous form of the cdf and the quantile 
function. These two aliases (in particular \code{q.l}) do work in \texttt{RStudio}.
\end{small}
\par
\begin{small}
\noindent{\bf Comment:}\\
