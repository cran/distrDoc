...
\end{footnotesize}
<<SimulateandEstimate, eval = TRUE, fig = TRUE>>=
...
setMethod("convpow",
    ...
    ##STEP 1
            ...
            upper <- ifelse((q(D1)(1) < Inf), q(D1)(1), 
                     q(D1)(1-getdistrOption("TruncQuantile")))
    ...
    ##STEP 2
            ...
            h <- (upper-lower)/M
            if(h > 0.01)
              warning(paste("Grid for approxfun too wide, ", "increase DefaultNrFFTGridPointsExponent", sep=""))
